      ...
      if (interaction.commandName === "doi") {
        try {
          ...
          const metadata = await getPaperMetadata(
            (interaction.options.getString("doi") as string).trim()
          );
          ...
          const embed: EmbedBuilder = new EmbedBuilder()
            ...
            .addFields(
              ...
              {
                name: metadata.authors.length === 1 ? "Author" : "Authors",
                value: metadata.getAuthors(transformer),
              }
            );
