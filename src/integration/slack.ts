...
export function runSlackServer() {
  ...
  const requestListener = (
    ...
    if (req.url === "/doi" && req.method === "POST") {
      ...
      req.on("end", async () => {
        ...
        try {
          // Get paper metadata
          const metadata = await getPaperMetadata(
            decodeURIComponent(data["text"]).trim()
          );
          ...
          if (metadata.abstract && metadata.abstract.length > 350) {
            ...
            metadata.abstract = `${metadata.abstract.slice(
              0,
              metadata.abstract.lastIndexOf(" ", 347)
            )}...`;
          }
  ...
  server.listen(parseInt(process.env.SLACK_PORT), process.env.SLACK_HOST, () =>
    console.log(
      `Server is running on http://${process.env.SLACK_HOST}:${process.env.SLACK_PORT}`
    )
  );
}

// Run slack server if this is the main file
